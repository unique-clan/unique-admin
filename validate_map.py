...
FRONT_TILES  = list(map(TILEINDEX.get, ['air', 'death', 'start', 'finish', \
              ...
              'stopper', 'stopper_twoway', 'stopper_allway'])) + ENTITIES_OFF_SIGN + [5] # hookthrough
...
def validate_map(path, gtype, only_critical=False):
    ...
    if show_error:
        # The conversion tool reports if tile layer image size is not multiple of 16 because this is not supported by 0.7 clients.
        p = subprocess.run(['./map_convert_07', mappath, '/dev/null'], capture_output=True, text=True)
